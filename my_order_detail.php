...
%>*/
$query_orderlist=  "select B.OrderTime,B.OrderNum,B.pei,B.fei,B.Memo,B.thiskou,B.RecName,B.RecPhone,B.RecMail,B.RecAddress,B.ZipCode,B.PayType,B.Discount,B.Notes,B.OrderSum,B.PayStatus,D.UserKou,B.Status,B.SellerRating
...
<tr><td colspan="4"><fieldset class="rating">
    <legend>評價賣家</legend>
    <form id="sellerRating-UpdateForm" action="#" method="post">
        <input type="hidden" name="OrderNum" value="<?= $rs_orderlist['OrderNum'] ?>" />
        <input type="radio" id="star5" name="rating" value="5" <?php if ($rs_orderlist['SellerRating'] === '5'): ?>checked<?php endif; ?>/><label for="star5" title="超讚">5星</label>
        <input type="radio" id="star4" name="rating" value="4" <?php if ($rs_orderlist['SellerRating'] === '4'): ?>checked<?php endif; ?>/><label for="star4" title="還不錯">4星</label>
        <input type="radio" id="star3" name="rating" value="3" <?php if ($rs_orderlist['SellerRating'] === '3'): ?>checked<?php endif; ?>/><label for="star3" title="普普通通">3星</label>
        <input type="radio" id="star2" name="rating" value="2" <?php if ($rs_orderlist['SellerRating'] === '2'): ?>checked<?php endif; ?>/><label for="star2" title="有點糟">2星</label>
        <input type="radio" id="star1" name="rating" value="1" <?php if ($rs_orderlist['SellerRating'] === '1'): ?>checked<?php endif; ?>/><label for="star1" title="遜斃了">1星</label>
    </form>
</fieldset></td></tr>
<?php endif; ?>
